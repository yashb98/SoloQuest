...
import { Clock, Trophy, Swords, CheckCircle, Undo2 } from "lucide-react";
...
export default function DungeonsPage() {
  ...
  const handleAction = async (action: string, dungeonId: number) => {
    ...
    if (data.success && action === "uncomplete") {
      // Dungeon undo succeeded — list will refresh below
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-sq-muted">Weekly Challenges</span>
      </div>
      ...
      {activeDungeon && (
        <motion.div
          ...
          className="sq-panel p-5 border-2 border-sq-purple/50 shadow-[0_2px_8px_rgba(168,85,247,0.3)]"
        >
          <div className="flex items-center gap-2 mb-3">
            <Swords className="w-5 h-5 text-sq-purple" />
            <span className="font-bold text-sq-purple">ACTIVE DUNGEON</span>
          </div>
          <h3 className="font-bold text-lg text-sq-text">{activeDungeon.title}</h3>
          <p className="text-xs text-sq-muted mt-1">{activeDungeon.description}</p>
          ...
          <div className="mt-3 space-y-1">
            {JSON.parse(activeDungeon.objectives).map((obj: string, i: number) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className="text-xs text-sq-text">{obj}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button
              ...
              className="px-4 py-2 border border-red-500/30 text-red-400 rounded-md text-sm font-bold hover:bg-red-500/10"
            >
              ABANDON
            </button>
          </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {dungeons.filter((d) => !d.isActive).map((dungeon) => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-sm text-sq-text">{dungeon.title}</h3>
                {dungeon.isCompleted && (
                  <div className="flex items-center gap-1.5">
                    <CheckCircle className="w-4 h-4 text-sq-green" />
                    <button
                      onClick={() => handleAction("uncomplete", dungeon.id)}
                      disabled={actionId === dungeon.id}
                      className="p-1 rounded-lg text-sq-muted hover:text-red-500 hover:bg-red-50 transition-colors"
              ...
              <p className="text-[11px] text-sq-muted">{dungeon.description}</p>
              <div className="flex items-center gap-3 text-xs">
                <span className="text-sq-gold">+{dungeon.bonusXP} XP</span>
                <span className="text-sq-gold">+{dungeon.bonusGold} G</span>
                <span className="text-sq-purple">+{dungeon.statAmount} {statIcons[dungeon.statReward] || dungeon.statReward}</span>
              </div>
              <div className="space-y-1">
                {objectives.slice(0, 3).map((obj: string, i: number) => (
                  <span key={i} className="text-[10px] text-sq-muted block">• {obj}</span>
                ))}
              </div>
              {!dungeon.isCompleted && !activeDungeon && (
                dungeon.canActivate ? (
                  ...
                  <div className="w-full text-center py-2.5 rounded-xl bg-sq-hover text-sq-muted text-sm font-semibold">
        ...
        {showResult && (
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-sq-gold">+{showResult.bonusXP} XP | +{showResult.bonusGold} Gold</p>
